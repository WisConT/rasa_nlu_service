...
def combine_tsv(outname, microposts_dir):
    extension = 'gold.tsv'
    all_filenames = [Path(data_external_dir)/microposts_dir/f for f in os.listdir(Path(data_external_dir)/microposts_dir) if (extension in f)]
    combined_csv = pd.concat([pd.read_csv(f, sep='\t', header=None,names=["id", "start", "end", "entry","entity"]) for f in all_filenames ])
    combined_csv.to_csv(Path(data_interim_dir)/"microposts2015"/outname, index=False, encoding='utf-8-sig', sep='\t')
    ...
    with open(fullname) as f:
        for idx, line in enumerate(f):
            ...
            if text not in examples:
                examples[text] = {"entities": [], "intent": "greeting", "text": text}
            start = int(split[1])
            end = int(split[2])

            examples[text]["entities"].append({"entity": split[-1], "start": start, "end": end, "value": text[start:end]})
