...
def extract_entities(ref_map, ent_mentions, ent_instances, sentence, xml):
    ...
    for em in ent_mentions:
        ...
        end = sum(lens[start_index:end_index + 1]) + end_index - start_index + start
        ...
        for em in all_ent_mentions:
            for an in em:
                if an.attrib["t_id"] in tids:
                    ...
                    continue
        ents, text = extract_entities(ref_map, set(ent_mentions), ent_instances, sent, xml)
        example = {"intent": "greeting", "entities": ents, "text": text, "xml": str(xml)}
