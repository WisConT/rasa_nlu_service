    ...
    with open(os.path.join(output_path, meta_file), 'wb') as file_metadata:
        for i, (key, vector) in enumerate(vectors.items()):
            ...
            file_metadata.write("{0}\n".format(
                '<Space>' if text. lstrip() == '' else text).encode('utf-8'))
...
@plac.annotations(
    vectors_loc=("Path to spaCy model that contains word vectors",
                 "positional", None, str),
    out_loc=("Path to output tensorboard vector visualization data",
             "positional", None, str),
    name=("Human readable name for tsv file and tensor name",
          "positional", None, str),
)
def main(vectors_loc, out_loc, name="spaCy_vectors"):
    ...
    print(
        'Done. Run `tensorboard --logdir={0}` to view in Tensorboard'.format(out_loc))
