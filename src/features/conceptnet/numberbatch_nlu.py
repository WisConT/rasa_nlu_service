...
class SpacyNLP(Component):
    ...
    @classmethod
    def create(
        cls, component_config: Dict[Text, Any], config: RasaNLUModelConfig = None
    ) -> "SpacyNLP":
        import spacy
        ...
        # logger.info(
        #     "Trying to load spacy model with name '{}'".format(
        #         spacy_model_name)
        # )

        nlp = spacy.load(
            'data/embeddings/numberbatch/numberbatch_spacy', disable=["parser"])
        cls.ensure_proper_language_model(nlp)
        return cls(component_config=component_config, nlp=nlp)

    @classmethod
    def cache_key(
        cls, model_metadata: "Metadata", component_meta: Dict[Text, Any] = None
    ) -> Optional[Text]:
